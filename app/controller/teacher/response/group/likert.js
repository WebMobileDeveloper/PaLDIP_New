(function () {
	...
	function responseOfLikertAnswerController($state, $scope, $rootScope, $filter) {
		...
		$scope.getGroupData = function () {
			...
			$scope.groupDataRef.on('value', function (snapshot) {
				...
				if ($scope.groupData.groupLikertSettings && $scope.groupData.groupLikertSettings.group && $scope.groupData.groupLikertSettings.group[$scope.setKey]) {
					...
					if (setSetting.showResponse) $scope.groupLikertSettings.showResponse = true
		...
		$scope.getSiblingAnswers = function (siblingSetIndex, currQSTAnswers) {
			...
			$scope.calcValues[siblingSetIndex] = angular.copy(classAnswers)
			calcValues();
		}
		$scope.getAnswers = function () {
			...
			if ($scope.groupLikertSettings.showOther) {
				$scope.calcValues[labelCount] = angular.copy(allAnswers);
				$scope.userKeys[labelCount] = Array(xLength).fill("All Class Average");
		...
		calcValues = function () {
			if ($scope.allSetsCount < $scope.siblingSets.length) {
				$scope.allSetsCount++
				return
			}
		...
		$scope.saveLikertSettings = function () {
			...
			let hideOthers = $scope.tempSettings.showOther ? {} : true
			...
			let showVersion = $scope.tempSettings.showVersion ? true : {}
			let newData = {
			...
			firebase.database().ref('Groups/' + $rootScope.settings.groupKey +
				'/groupLikertSettings/group/' + $scope.setKey).set(newData).then(() => {
					$('#editLikertInfoModal').modal('hide');
