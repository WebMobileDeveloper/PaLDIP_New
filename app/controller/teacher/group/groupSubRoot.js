(function () {
    ...
    function groupSubRootController($state, $scope, $rootScope, $filter) {
        ...
        $scope.getLinks = function () {
            ...
            $scope.linkRef.on('value', function (snapshot) {
                ...
                for (key in $scope.allLinks) {
                    ...
                    if (link.groupKey == $scope.groupKey && link.groupType == 'sub' && 
        ...
        $scope.finalCalc = function () {
            ...
            for (var key in $scope.groupset.sections) {
                let section = $scope.groupset.sections[key];
                // =====  item data========
                section.questions = [];
                // ------------------------
                $scope.QuestionSets.push(section);
            }
            for (var key in $scope.groupset.QuestionSets) {
                let set = $scope.groupset.QuestionSets[key]
